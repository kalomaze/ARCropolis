...
pub fn perform_discovery() -> LaunchPad<StandardLoader> {
    ...
    if storage.get_flag("first_boot") {
        if skyline_web::Dialog::yes_no("A default configuration for ARCropolis has been created.<br>It is important that the region language matches the one you configured for the game.<br>By default, it is set to American English. Would you like to adjust it?") {
